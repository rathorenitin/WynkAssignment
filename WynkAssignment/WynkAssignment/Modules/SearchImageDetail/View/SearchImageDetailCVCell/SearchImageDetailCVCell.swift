...
class SearchImageDetailCVCell: UICollectionViewCell {
    ...
    func configureData(with imageData: Hit?) {
        guard let imgD = imageData else {return}
        self.image = #imageLiteral(resourceName: "ic_placeholder")
        
        ImageDownloader.shared.downloadImage(with: imgD.largeImageURL, completionHandler: { [weak self] (image, cache) in
            if let image = image{
                self?.image = image
...
extension SearchImageDetailCVCell: UIScrollViewDelegate {
